import { EmployeeSample, EventMap, RoleSample } from "./module.js";
...
export async function toggleStatus(e: Event): Promise<void> {
    let selectedCell = e.target as HTMLElement;
    let selectedRow: HTMLElement = selectedCell.parentElement!.parentElement!.parentElement!;
    let selectedRowEmpId: string = selectedRow.querySelector<HTMLTableElement>('.employee-no')!.innerText;
    let selectedEmp: EmployeeSample | undefined;
    let statusElement: HTMLSpanElement = selectedRow.querySelector(".employee-status-value")!;
    let isActive: boolean = statusElement.innerText === "Active";
    ...
    (statusElement.innerText == "Active") ? selectedCell.innerText = 'Mark as In active' : selectedCell.innerText = 'Mark as Active';
    filterSearch();
}
...
function deleteEmployee(row: HTMLTableRowElement) {
    ...
    let selectedEmp:EmployeeSample | undefined=employeeList.find(employee => employee.id==rowEmpId);
    let newEmps: EmployeeSample[] = employeeList.filter(employee => employee.id !== rowEmpId);
