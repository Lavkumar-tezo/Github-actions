  ...
  xhr.onload = () => {
    ...
    if (employeeList) {
      ...
      if (selectedEmp) {
        ...
        let selectedEmpRole: RoleSample[] = allRoles.filter((role: RoleSample) => role.id == selectedEmp?.role)
...
export function addEmployee(event: Event, mode: string): void {
  ...
  if (check) {
    if (mode == "edit") {
      ...
      // let rowIndex:number = employeeList.findIndex( employee => employee.id == rowId);
      // document.querySelector<HTMLTableElement>('.employee-table-body')!.deleteRow(rowIndex + 1);
      let newEmps: EmployeeSample[] = employeeList.filter(employee => employee.id !== rowId);
