...
document.addEventListener('DOMContentLoaded', () => {
    ...
    xhr2.onload = () => {
        ...
        allRoles.forEach(role => {
            ...
            role.profiles = [];
            let filteredEmp: EmployeeSample[] = employeeList.filter((employee) => employee.role == roleId)
            role.profiles = filteredEmp;
            createRoleCard(role);
        })
    }
    xhr2.send();
...
export function createRoleCard(data: RoleSample): void {
    ...
    if (data.profiles) {
        ...
        for (let i = 0; i < minimumProfie; i++) {
            ...
            let profileEmpNo: string = data.profiles[i].id;
            let emp: EmployeeSample[] = employeeList.filter(employee => employee.id == profileEmpNo);
            if (emp.length == 0) {
...
export function createDivBlock(element: HTMLElement): void {
    let selectedRoleTitle: string | null = localStorage.getItem('selectedRole');
    if(selectedRoleTitle)
        selectedRoleTitle=JSON.parse(selectedRoleTitle);
    let xhr2 = new XMLHttpRequest();
    xhr2.open('GET', "http://localhost:3000/employeeList", false);
    ...
    xhr.onload = function () {
        ...
        let selectedRole: RoleSample[] = allRoles.filter(role => role.role == selectedRoleTitle);
        if (selectedRole) {
            employeeList.forEach(emp => {
                if (emp.role == '')
                    createEmployeeDiv(emp, element, false)
                if (selectedRoleTitle && (selectedRoleTitle.length != 0) && (emp.role == selectedRole[0].id)) {
